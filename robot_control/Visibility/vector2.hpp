...
struct vector2 {
  ...
  using if_vector = typename std::enable_if<!std::is_scalar<VectorType>::value,
											ReturnType>::type;

  template <typename ScalarType, typename ReturnType>
  using if_scalar = typename std::enable_if<std::is_scalar<ScalarType>::value,
											ReturnType>::type;
  ...
  template <typename ScalarType>
  friend if_scalar<ScalarType, vector2> operator*(ScalarType scalar,
												  vector2 vector) {
  ...
  friend vector2<T> operator-(vector2<T> vec) { return {-vec.x, -vec.y}; }
