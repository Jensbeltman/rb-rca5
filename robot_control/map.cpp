...
void Map::show() {
  ...
  for(int i = 0; i < nEst; i++){
      arrowedLine(m, rConf[i].pos, rConf[i].pos + Point2f(8 * cos(rConf[i].dir), 8 * sin(rConf[i].dir)), Scalar(120, 120, 255), 1, LINE_AA, 0, .5);
  }
  arrowedLine(m, rConf[0].pos, rConf[0].pos + Point2f(8 * cos(rConf[0].dir), 8 * sin(rConf[0].dir)), Scalar(0, 255, 0), 1, LINE_AA, 0, .5);
  arrowedLine(m, pos, pos + Point2f(8 * cos(dir), 8 * sin(dir)), Scalar(255, 0, 0), 1, LINE_AA, 0, .5);
